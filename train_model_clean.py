        ...
        for phase in ['train', 'val']:
            ...
            for i, data in enumerate(dataloaders[phase]):
                ...
                if phase == 'train':
                    ...
                    # Convert class index labels to one-hot encoded labels
                    one_hot_labels = torch.nn.functional.one_hot(labels.to(torch.int64), num_classes)
                    uniform_labels = torch.full((args.batch_size, num_classes), 1.0/num_classes, dtype = torch.float)
                    print(uniform_labels[0][0], uniform_labels[0][0])

                    print(outputs[0][0], one_hot_labels[0][0])
                    loss2 = criterion(outputs.float(), one_hot_labels.float())
                    ...
                    with torch.no_grad():
                        ...
                        loss2 = criterion(outputs.cpu(), one_hot_labels)
                        #loss = loss2
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i, data in enumerate(dataloaders[phase]):
                ...
                if phase == 'train':
                    model.train()
                    inv_1, inv_2, outputs = model(inputs, labels) # forward
...
def fit_freeze(args, examples_training, labels_training):
    ...
    for k, data_test_0 in enumerate(test_0_loader):
        ...
        _, _, outputs_test_0 = model(concat_input, labels = None)
